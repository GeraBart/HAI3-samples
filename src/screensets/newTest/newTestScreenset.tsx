...
import { initializeNewTestEffects } from './effects/newTestEffects';

// Import for side effect - register API service
import { NEW_TEST_DOMAIN } from './api/newTestApiService';
import './api/newTestApiService';
import { newTestMockMap } from './api/mocks';

// Register mock data for API service
apiRegistry.registerMocks(NEW_TEST_DOMAIN, newTestMockMap);
 ...
 */
registerSlice(newTestSlice, (dispatch) => {
  initializeNewTestEffects(dispatch);
});

/**
 * newTest Screenset Configuration
 */
export const newTestScreenset: ScreensetConfig = {
  id: NEW_TEST_SCREENSET_ID,
  name: 'newTest',
  category: ScreensetCategory.Mockups,
  defaultScreen: OVERVIEW_SCREEN_ID,
  localization: screensetTranslations,
  // Keep one menu item but we'll use custom SidebarSections for the full navigation
  menu: [
    {
      menuItem: {
        id: OVERVIEW_SCREEN_ID,
        label: `screenset.${NEW_TEST_SCREENSET_ID}:menu_items.overview.label`,
        icon: HOME_ICON_ID,
      },
      screen: () => import('./screens/home/OverviewScreen'),
    },
  ],
};
 ...
 */
screensetRegistry.register(newTestScreenset);
